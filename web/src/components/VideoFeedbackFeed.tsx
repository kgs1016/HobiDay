...
import { feedbackMediaUrls, fetchFeedbackVideos } from "@/lib/feedbackVideo";

export default function VideoFeedbackFeed() {
  ...
  const load = async (before?: VideoSummary) => {
    ...
    setError("");
    try {
      const next = await fetchFeedbackVideos(before);
  ...
  useEffect(() => {
    let alive = true;
    fetchFeedbackVideos().then(async (next) => {
      const thumbnails = await feedbackMediaUrls(next.map((r) => r.thumbnail_path));
      if (!alive) return;
  ...
  return (
    <section className="pb-8 pt-4" aria-label="영상 피드백 목록">
      ...
      {loaded && !rows.length && !error && <div className="py-16 text-center">
        <p className="text-[15px] font-semibold">아직 올라온 영상이 없어요</p>
        <Link href="/community/upload" className="mt-4 inline-block rounded-xl bg-accent px-5 py-3 text-sm font-semibold text-white">첫 영상 올리기</Link>
      </div>}
