...
export default function ShoeCriteria({ current }: { current?: ShoeColorId }) {
  return <Link href={`/me/grades${current ? `?stage=${current}` : ""}`}
    className="mt-1 flex min-h-12 items-center justify-between gap-3 text-[13px] font-medium text-muted">
    단계 기준 보기<ChevronRightIcon size={16} className="text-faint" />
  </Link>;
}
