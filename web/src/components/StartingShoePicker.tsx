...
export default function StartingShoePicker({ onboarding = false, reset = false, skipLabel, onSkip, onSaved, onBusyChange }: {
  ...
  return <>
    ...
    <p aria-live="polite" className="mt-3 min-h-9 text-center text-[12px] text-muted">
      {stage ? stage.minLevel ? `유지 기준 H${stage.minLevel} 이상 ${stage.required}개 · ${stage.points.toLocaleString()}점` : "기록과 함께 시작하는 흰색 클라이밍화" : "평소 완등하는 난이도를 참고해 선택해주세요"}
    </p>
    ...
    <button type="button" onClick={onSkip} disabled={busy} className="mt-1 min-h-11 w-full text-[13px] text-muted">{skipLabel ?? (reset ? "취소" : "나중에 설정")}</button>
  </>;
}
