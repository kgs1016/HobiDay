...
type Filters = { topic: BoardTopic | null; query: string; hot: boolean };
const TOPIC_TABS: { id: BoardTopic | null; label: string; hot: boolean }[] = [
  { id: null, label: "전체", hot: false },
  { id: null, label: "HOT", hot: true },
  ...BOARD_TOPICS.map(t => ({ ...t, hot: false })),
];

/** 필터마다 다시 마운트해 이전 요청·커서가 다른 주제의 목록에 섞이지 않게 한다. */
function FilteredPosts({ topic, query, hot, onPins }: Filters & { onPins: (pins: BoardFeedPage["pinned"]) => void }) {
  const mock = !hasSupabase();
  const [rows, setRows] = useState<PostSummary[] | null>(mock ? mockBoardFeed(topic, query, hot).items : null);
  ...
  const loadMore = async () => {
    ...
    try {
      const page = await fetchBoardFeed(topic, query, hot, rows?.at(-1));
      if (!page) throw new Error("load_failed");
      const urls = await signedPhotoUrls(page.items.map(p => p.photo).filter(Boolean) as string[]);
      ...
      setMore(!hot && page.items.length === POST_PAGE);
    } catch {
  ...
  return <div aria-busy={busy}>
    ...
    {rows?.length === 0 && !error && <div className="py-14 text-center">
      <p className="text-[14px] font-medium text-muted">{query ? "검색 결과가 없어요" : hot ? "아직 HOT 글이 없어요" : topic ? "아직 이 주제의 글이 없어요" : "아직 글이 없어요"}</p>
    </div>}
...
export default function FreeBoardFeed({ topic, query, hot, onChange }: Filters & { onChange: (filters: Filters) => void }) {
  ...
  return <section aria-label="자유게시판 목록" className="pb-6">
    ...
    <div className="mt-4 flex items-center gap-2">
      <div className="flex min-w-0 flex-1 gap-1.5 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" aria-label="글 주제">
        {TOPIC_TABS.map((t, index) => <button type="button" key={t.hot ? "hot" : t.id ?? `all-${index}`}
          onClick={() => onChange({ topic: t.id, query, hot: t.hot })} aria-pressed={hot === t.hot && topic === t.id}
          className={`min-h-9 shrink-0 rounded-full px-3 text-[12.5px] font-medium transition-colors ${hot === t.hot && topic === t.id ? "bg-ink text-white" : "bg-surface2 text-muted"}`}>
          {t.label}
        </button>)}
      </div>
      <button type="button" aria-label={searchOpen ? "글 검색 닫기" : "글 검색"} aria-expanded={searchOpen} aria-controls="board-search"
        onClick={() => { if (searchOpen) { setDraft(""); onChange({ topic, query: "", hot }); } setSearchOpen(!searchOpen); }}
    ...
    {searchOpen && <form id="board-search" role="search" className="mt-3 flex items-center gap-2 rounded-xl bg-surface2 px-3"
      onSubmit={event => { event.preventDefault(); onChange({ topic, query: draft.trim().slice(0, 80), hot }); }}>
      <input type="search" value={draft} maxLength={80} onChange={event => {
        setDraft(event.target.value);
        if (!event.target.value && query) onChange({ topic, query: "", hot });
      }}
    ...
    <div className="mt-4 flex items-center justify-between border-b border-line pb-2.5 text-[11.5px] text-faint">
      ...
      <span className="ml-3 shrink-0">{hot ? "최근 7일" : "최신순"}</span>
    </div>
    <FilteredPosts key={`${hot ? "hot" : topic ?? "all"}:${query}`} topic={topic} query={query} hot={hot} onPins={setPins} />
  </section>;
}
