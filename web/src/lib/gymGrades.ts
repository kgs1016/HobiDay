...
export const GYM_GRADE_GUIDES: GymGradeGuide[] = [
  ...
  },
  {
    ...
    sources: [{label:"메트로락 암장 안내표 · 이용자 재게시",url:"https://spiri7.com/post/479",published:null,checked:"2026-09-09"}],
    note: "공개된 암장 안내표 기준입니다. 원본 작성일은 확인되지 않아 현장 안내가 다르면 직접 입력해주세요. 핑크는 하강용 홀드로, 난이도 색상이 아닙니다.",
  },
  {
  ...
  },
  {
    ...
    colors: [{"name":"흰색","hex":"#F1F0EB"},{"name":"노랑","hex":"#E9BF3A"},{"name":"연두","hex":"#B2CC66"},{"name":"초록","hex":"#60A783"},{"name":"파랑","hex":"#5F95D4"},{"name":"빨강","hex":"#DF5957"},{"name":"회색","hex":"#969BA3"},{"name":"갈색","hex":"#9A745C"},{"name":"핑크","hex":"#ED9FBF"}],
    sources: [
  ...
  },
  {
    ...
    colors: [{"name":"흰색","hex":"#F1F0EB"},{"name":"노랑","hex":"#E9BF3A"},{"name":"초록","hex":"#60A783"},{"name":"파랑","hex":"#5F95D4"},{"name":"빨강","hex":"#DF5957"},{"name":"검정","hex":"#343B45"},{"name":"회색","hex":"#969BA3"},{"name":"갈색","hex":"#9A745C"},{"name":"핑크","hex":"#ED9FBF"}],
    sources: [
  ...
  },
  {
    ...
    colors: [{"name":"빨강","hex":"#DF5957"},{"name":"주황","hex":"#ED9354"},{"name":"노랑","hex":"#E9BF3A"},{"name":"초록","hex":"#60A783"},{"name":"파랑","hex":"#5F95D4"},{"name":"남색","hex":"#526296"},{"name":"보라","hex":"#A287C4"},{"name":"회색","hex":"#969BA3"},{"name":"검정","hex":"#343B45"},{"name":"핑크","hex":"#ED9FBF"}],
    sources: [
...
// 기존 지점별 방문 이력을 같은 브랜드 하나로 합친다.
const RECENT_KEY = "hobiday:grade-guide-recent";
export function readRecentGradeGyms(): string[] {
  try {
    const value: unknown = JSON.parse(window.localStorage.getItem(RECENT_KEY) ?? "[]");
    if (!Array.isArray(value)) return [];
    const names = value.flatMap(item => typeof item === "string" ? findGymGradeGuide(item)?.name ?? [] : []);
    return [...new Set(names)].slice(0, 3);
  } catch { return []; }
}
export function rememberGradeGym(name: string): string[] {
  const guide = findGymGradeGuide(name);
  if (!guide) return readRecentGradeGyms();
  const recent = [guide.name, ...readRecentGradeGyms().filter(item => item !== guide.name)].slice(0, 3);
  try { window.localStorage.setItem(RECENT_KEY, JSON.stringify(recent)); } catch { /* 저장이 차단돼도 안내는 볼 수 있다. */ }
  return recent;
}
