...
(async () => {
  ...
  try {
    ...
    await db.exec(table(read('20260910050000_profile_frequency_and_shoe_start.sql').replace('create table climbing_shoe_starts','create table if not exists climbing_shoe_starts'),'climbing_shoe_starts'));
    await db.query("insert into climbing_shoe_starts(user_id,stage,expires_on) values($1,'white',current_date+90)",[id(1)]);
    await db.query('update profiles set career=1 where id=$1',[id(1)]);
    ...
    await assert.rejects(db.query('update profiles set is_public=true where id=$1',[id(2)]),/profile_incomplete|profiles_public_needs_basic_info/);
    // Null demographic values cannot pass the public-profile constraint.
    await db.query("update profiles set gender='m',career=1,age=null,photo=null where id=$1",[id(2)]);
    assert.equal(await scalar('select my_participation_profile_ready()'),false,'shoe choice is also required');
    await assert.rejects(create(),/profile_incomplete/);
    await db.query("insert into climbing_shoe_starts(user_id,stage,expires_on) values($1,'white',current_date+90)",[id(2)]);
    await db.query('update profiles set is_public=true where id=$1',[id(2)]);
    ...
    await db.query("update profiles set gender='f',career=2,age=null,photo=null where id=$1",[id(3)]);
    await db.query("insert into climbing_shoe_starts(user_id,stage,expires_on) values($1,'pink',current_date-1)",[id(3)]);
    ...
    await db.exec('reset role');
    await db.query('update profiles set career=null where id=$1',[id(3)]);
